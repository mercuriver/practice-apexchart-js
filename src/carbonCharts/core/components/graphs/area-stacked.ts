...
export class StackedArea extends Component {
  ...
  render(animate = true) {
    ...
    const domainAxisPosition =
      this.services.cartesianScales.getDomainAxisPosition({ firstDatum });
    const rangeAxisPosition =
      this.services.cartesianScales.getRangeAxisPosition({ firstDatum });
    const mainYScale =
      this.services.cartesianScales.getScaleByPosition(rangeAxisPosition);
  ...
  handleLegendOnHover = (event: CustomEvent) => {
    ...
    this.parent
      ...
      .attr("opacity", (d) => {
        if (
          Tools.getProperty(d, 0, groupMapsTo) !== hoveredElement.datum().name
        ) {

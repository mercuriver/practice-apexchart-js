...
export class Scatter extends Component {
  ...
  filterBasedOnZoomDomain(data) {
    const domainIdentifier =
      this.services.cartesianScales.getDomainIdentifier(data);
      ...
      scatterData = this.model
        ...
        .filter((d) => {
          const rangeIdentifier =
            this.services.cartesianScales.getRangeIdentifier(d);
          return (
            d[rangeIdentifier] !== undefined && d[rangeIdentifier] !== null
          );
        });
    }
  ...
  styleCircles(selection: Selection<any, any, any, any>, animate: boolean) {
    ...
    selection
      ...
      .style("fill", (d) => {
        const domainIdentifier = cartesianScales.getDomainIdentifier(d);
        if (
          this.model.getIsFilled(d[groupMapsTo], d[domainIdentifier], d, filled)
        ) {
  ...
  handleChartHolderOnMouseOut = (event: CustomEvent) => {
    this.parent
      .selectAll("circle.dot")
      .transition(
        this.services.transitions.getTransition("chart-holder-mouseout-scatter")
      )
      .attr("opacity", 0);
  };
  ...
  handleLegendMouseOut = (event: CustomEvent) => {
    this.parent
      .selectAll("circle.dot")
      .transition(
        this.services.transitions.getTransition("legend-mouseout-scatter")
      )
      .attr("opacity", 1);
  };
    ...
    this.parent
      ...
      .on("mouseover", function (event, datum) {
        ...
        hoveredElement
          ...
          .style("fill", (d) => {
            const domainIdentifier =
              self.services.cartesianScales.getDomainIdentifier(d);
      ...
      .on("mouseout", function (event, datum) {
        ...
        if (!self.configs.filled) {
          ...
          const domainIdentifier =
            self.services.cartesianScales.getDomainIdentifier(datum);

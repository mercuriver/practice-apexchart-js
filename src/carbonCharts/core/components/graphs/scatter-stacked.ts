  ...
  render(animate: boolean) {
    ...
    const circlesToStyle = enteringCircles.merge(circles).datum((d) => {
      ...
      const domainIdentifier =
        this.services.cartesianScales.getDomainIdentifier(d);
  ...
  getTooltipData(hoveredX, hoveredY) {
    ...
    stackedData.forEach((groupData, groupDataIndex) => {
      groupData.forEach((datum, dataIndex) => {
        ...
        const domainIdentifier =
          this.services.cartesianScales.getDomainIdentifier(datum);
        const rangeIdentifier =
          this.services.cartesianScales.getRangeIdentifier(datum);

        if (
          rangeValue !== null &&
          rangeValue !== undefined &&
          hoveredX ===
            this.services.cartesianScales.getDomainValue(domainValue) &&
          hoveredY ===
            this.services.cartesianScales.getRangeValue(stackedRangeValue)
        ) {
          if (percentage) {
    ...
    return this.model
      ...
      .filter((datapoint) => {
        const domainIdentifier =
          this.services.cartesianScales.getDomainIdentifier(datapoint);
        const rangeIdentifier =
          this.services.cartesianScales.getRangeIdentifier(datapoint);
        return (

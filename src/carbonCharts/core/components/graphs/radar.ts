...
export class Radar extends Component {
  ...
  render(animate = true) {
    ...
    const horizSpaceNeededByEachXLabel = this.uniqueKeys.map((key) => {
      ...
      const distanceFromDiameter =
        distanceBetweenPointOnCircAndVerticalDiameter(xScale(key), radius);
          ...
          .call((selection) =>
            selection
              ...
              .attr(
                "x1",
                (key) =>
                  polarToCartesianCoords(xScale(key), yScale.range()[0], c).x
              )
              .attr(
                "y1",
                (key) =>
                  polarToCartesianCoords(xScale(key), yScale.range()[0], c).y
              )
              .attr(
                "x2",
                (key) =>
                  polarToCartesianCoords(xScale(key), yScale.range()[1], c).x
              )
              .attr(
                "y2",
                (key) =>
                  polarToCartesianCoords(xScale(key), yScale.range()[1], c).y
              )
          ),
      (update) =>
        update.call((selection) =>
    ...
    xLabelUpdate.join(
      (enter) =>
        enter
          ...
          .style(
            "dominant-baseline",
            (key) => radialLabelPlacement(xScale(key)).dominantBaseline
          )
          .call((selection) =>
            selection
              .transition(
    ...
    blobUpdate.join(
      ...
      (exit) =>
        exit.call((selection) => {
          const selectionUpdate = selection.transition(
            this.services.transitions.getTransition("radar_blobs_exit", animate)
          );
    ...
    dotsUpdate
      ...
      .attr(
        "cx",
        (d) => polarToCartesianCoords(xScale(d[angle]), yScale(d[value]), c).x
      )
      .attr(
        "cy",
        (d) => polarToCartesianCoords(xScale(d[angle]), yScale(d[value]), c).y
      )
      .attr("r", 0)
      .attr("opacity", 0)
      ...
      (enter) =>
        enter
          ...
          .attr(
            "x",
            (tick) =>
              polarToCartesianCoords(-Math.PI / 2, yScale(tick), c).x +
              yLabelPadding
          )
          .attr(
            "y",
            (tick) => polarToCartesianCoords(-Math.PI / 2, yScale(tick), c).y
          )
  ...
  getLabelDimensions = (label: string) => {
    ...
    const { width, height } = DOMUtils.getSVGElementSize(tmpTickText.node(), {
      useBBox: true,
    });
    tmpTick.remove();
    return { width, height };

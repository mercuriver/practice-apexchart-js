...
export class Axis extends Component {
  ...
  render(animate = true) {
    ...
    const scale =
      this.services.cartesianScales.getScaleByPosition(axisPosition);
    ...
    if (
      ...
      if (tickRotation === TickRotations.ALWAYS) {
        ...
        if (scale.step) {
          ...
          const averageLetterWidth = mockTextPiece.node().getBBox().width;
          ...
          invisibleAxisRef.selectAll("g.tick").each(function () {
            ...
            if (
              ...
              lastStartPosition +
                selection.text().length * averageLetterWidth * 0.8 >=
                xTransformation
            ) {
              shouldRotateTicks = true;
            }

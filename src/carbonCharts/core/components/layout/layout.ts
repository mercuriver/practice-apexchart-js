...
export class LayoutComponent extends Component {
  ...
  getPreferedAndFixedSizeSum(): number {
    ...
    svg
      ...
      .filter((d: any) => {
        ...
        return (
          growth === LayoutGrowth.PREFERRED || growth === LayoutGrowth.FIXED
        );
      })
      .each(function (d: any) {
        sum += d.size;
  ...
  getNumOfStretchChildren(): number {
    ...
    return svg
      .selectAll(`div.layout-child-${this._instanceID}`)
      .filter(
        (d: any) => Tools.getProperty(d, "growth") === LayoutGrowth.STRETCH
      )
      .size();
  }
  ...
  render(animate = true) {
    ...
    enteringBoxes
      ...
      .attr(
        "class",
        (d: any) => `layout-child layout-child-${this._instanceID} ${d.id}`
      )
      .each(function (d: any) {
